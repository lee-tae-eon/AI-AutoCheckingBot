...
private:
    ...
    ros::Publisher obj_pub; // 추가 
    ros::Publisher arduino_pub; // 추가 
...
void ObjectDetectionGlRosNode::InitROS(void)
{
    ...
    data_pub = nh.advertise<sensor_msgs::LaserScan>(pub_topicname_lidar, 1);
    rviz_pub = nh.advertise<visualization_msgs::MarkerArray>(pub_topicname_marker, 1);
    obj_pub = nh.advertise<object_detection_gl_ros::obj_array>("position",1); // 추가 
    arduino_pub = nh.advertise<visualization_msgs::MarkerArray>("arduino",1); // 추가 
...
void ObjectDetectionGlRosNode::Run(void)
{
    ...
    {
        obj_detect.Detection(true);    object_detection_gl_ros::obj_array obj_msg;
        PubMarker();
    }
...
void ObjectDetectionGlRosNode::PubMarker(void) 
{
    ...
    // 아두이노를  위한 초기화 
    visualization_msgs::MarkerArray arduino_array_msg; 
    visualization_msgs::Marker arduino_msg; 
    ...
    object_detection_gl_ros::obj_position position_msg; // 초기화를 위한 추가 , 위치주의!!!
    object_detection_gl_ros::obj_array obj_msg; // 초기화를 위한 추가 
    //position_msg.action = object_detection_gl_ros::obj_position::DELETEALL;
    //obj_msg.xy.push_back(position_msg);
    ...
    for(int i=0; i<obj_detect.obj_info_vector.size(); i++)
    {
        ...
        //////////////////////////////////////////////////////////////
        visualization_msgs::Marker marker_msg;
	visualization_msgs::MarkerArray arduino_array_msg; //버퍼 
        visualization_msgs::Marker arduino_msg; //버퍼 
	...
	// x,y 좌표 저장 ////////////////////////////////////////
	//position_msg.action = object_detection_gl_ros::obj_position::ADD;
	//position_msg.x = obj_detect.obj_info_vector[i].cur_point[0];
	position_msg.x	= marker_msg.pose.position.x;
	position_msg.y	= marker_msg.pose.position.y;
	obj_msg.xy.push_back(position_msg); // 추가 
        obj_pub.publish(obj_msg); // 퍼블리싱
	////////////////////////////////////////////////////////

	// 아두이노 //
	arduino_msg.header.seq = 0;
	arduino_msg.header.frame_id = frame_id;
        arduino_msg.header.stamp = ros::Time::now();
        arduino_msg.ns = "obj";
        arduino_msg.id = obj_detect.obj_info_vector[i].id;
        arduino_msg.type = visualization_msgs::Marker::SPHERE;
        arduino_msg.action = visualization_msgs::Marker::ADD;
	arduino_msg.pose.position.x= marker_msg.pose.position.x;
	arduino_msg.pose.position.y = marker_msg.pose.position.y;
	arduino_msg.pose.position.z = 0;
        arduino_msg.pose.orientation.w = 1.0;
        arduino_msg.scale.x = 0.2;
        arduino_msg.scale.y = 0.2;
        arduino_msg.scale.z = 0.2;
        arduino_msg.color.a = 1.0; // Don't forget to set the alpha!
        arduino_msg.color.r = 0.0;
        arduino_msg.color.g = 1.0;
        arduino_msg.color.b = 0.0;
	arduino_array_msg.markers.push_back(arduino_msg);
	arduino_pub.publish(arduino_array_msg);
	//////////////////////////////////////////////////
	...
	marker_array_msg.markers.push_back(marker_msg);
        ...
        text_msg.pose.position.y = obj_detect.obj_info_vector[i].cur_point[1]+0.5; // +0.5 삭제 
	...
	//////////////////////////// 값 수정 cur_vel을 cur_point로 수정 //////////////////////////
	float x = obj_detect.obj_info_vector[i].cur_point[0];
	float y = obj_detect.obj_info_vector[i].cur_point[1];
	float distance = sqrt((x*x) + (y*y));

        std::string text_string = "[" + std::to_string(text_msg.id) + "]\nx,y=(" + std::to_string(obj_detect.obj_info_vector[i].cur_point[0]) + ", " + std::to_string(obj_detect.obj_info_vector[i].cur_point[1]) + ")\ndistance =(" + std::to_string(distance)+ ")";
	...
	///////////////////////////////////////////////////////////
        ...
        bool condition1 = obj_detect.obj_info_vector[i].total_point_count>obj_detect.max_mean_point;
